...
DINGTALK_API="https://oapi.dingtalk.com/robot/send?access_token=" # 需要自行替换
...
🕒 時間: $time"
            ...
            if [ -n "$TG_BOT_TOKEN" ] && [ -n "$TG_CHAT_IDS" ] && [ "$ENABLE_TG_NOTIFY" = "Y" ]; then
                send_tg "$message"
            fi
            if [ -n "$DINGTALK_WEBHOOK" ] && [ "$ENABLE_DINGTALK_NOTIFY" = "Y" ]; then
...
send_dingtalk() {
    ...
    if [ -z "$DINGTALK_WEBHOOK" ]; then
        echo -e "${RED}错误: 钉钉Webhook未配置${NC}"
        return 1
    fi
    
    # 构建钉钉消息的JSON格式
    dingtalk_message=$(cat <<-EOF
    {
        "msgtype": "markdown",
        "markdown": {
            "title": "VPS 通知",
    ...
    if ! echo "$response" | grep -q '"errcode":0'; then
        echo -e "${RED}发送钉钉通知失败: $response${NC}"
    else
        echo -e "${GREEN}成功发送钉钉通知${NC}"
    fi
}
...
print_menu_header() {
    clear
    echo -e "${CYAN}════════════════════════════════════════${NC}"
    echo -e "${CYAN}║       ${YELLOW}VPS 通知系統       ${CYAN}║${NC}"
    echo -e "${CYAN}════════════════════════════════════════${NC}"
    echo ""
}
...
show_config() {
    ...
    if [ -f "$CONFIG_FILE" ]; then
        ...
        if [ -n "$DINGTALK_WEBHOOK" ]; then
            # 掩盖Webhook的一部分，只显示开头和结尾部分
            webhook_prefix=$(echo "$DINGTALK_WEBHOOK" | cut -c -10)  # 获取前10个字符
            webhook_suffix=$(echo "$DINGTALK_WEBHOOK" | tail -c 10)   # 获取后10个字符
            webhook_masked="$webhook_prefix...$webhook_suffix"
            echo -e "${BLUE}钉钉 Webhook:${NC} $webhook_masked"
        else
            echo -e "${BLUE}钉钉 Webhook:${NC} ${RED}未设置${NC}"
        fi

        echo -e "${BLUE}主机备注:${NC} ${HOST_NOTE:-未设置}"        
        ...
        echo -e "${BLUE}启用TG通知:${NC} ${ENABLE_TG_NOTIFY:-Y}"
        echo -e "${BLUE}启用钉钉通知:${NC} ${ENABLE_DINGTALK_NOTIFY:-N}"
...
install_script() {
    ...
    echo -e "${CYAN}[1/8]${NC} 输入 Telegram Bot Token (留空跳过):"
    read -rp "Token (格式如123456789:ABCDEF...): " TG_BOT_TOKEN
    
    if [ -n "$TG_BOT_TOKEN" ]; then
        echo -e "\n${CYAN}[2/8]${NC} 输入接收通知的 Telegram Chat ID (支持多个，逗号分隔):"
    ...
    echo -e "\n${CYAN}[4/8]${NC} 是否要为这台主机添加备注? [Y/n] 默认不添加 (n): "
    read -rp "添加备注? " add_note
    add_note=${add_note:-n} # 默认n

    if [[ "$add_note" =~ ^[Yy]$ ]]; then
        echo -e "\n${CYAN}  请输入主机备注 (例如: 香港1号VPS):${NC}"
        read -rp "  备注: " HOST_NOTE
    fi
    
    echo -e "\n${CYAN}[5/8]${NC} 启用 SSH 登录通知? [Y/n]"
    ...
    while true; do
        ...
        case $choice in
            1)
                echo -e "\n${YELLOW}正在发送开机通知...${NC}"
                notify_boot
                echo -e "\n${GREEN}通知已发送，请检查你的Telegram 和 钉钉${NC}"
                read -rp "按Enter键继续..."
                ;;
            2)
...
modify_config() {
    ...
    while true; do
        ...
        echo -e "${CYAN}3.${NC} 修改 钉钉 Webhook"
        echo -e "${CYAN}4.${NC} 修改主机备注"
        echo -e "${CYAN}5.${NC} ${SSH_NOTIFY == "Y" ? "禁用" : "启用"} SSH登录通知"
        echo -e "${CYAN}6.${NC} ${ENABLE_MEM_MONITOR == "Y" ? "禁用" : "启用"} 内存监控 (当前阈值: ${MEM_THRESHOLD:-90}%)"
        echo -e "${CYAN}7.${NC} ${ENABLE_CPU_MONITOR == "Y" ? "禁用" : "启用"} CPU监控 (当前阈值: ${CPU_THRESHOLD:-4})"
        echo -e "${CYAN}8.${NC} ${ENABLE_IP_CHANGE_NOTIFY == "Y" ? "禁用" : "启用"} IP变动通知"
        echo -e "${CYAN}0.${NC} 返回主菜单"
        echo ""
        ...
        case $choice in
            1)
                echo -e "\n${YELLOW}请输入新的 Telegram Bot Token (留空则不修改):${NC}"
                ...
                ;;
            2)
                echo -e "\n${YELLOW}请输入新的 Telegram Chat ID(s) (多个ID用逗号分隔，留空则不修改):${NC}"
                ...
                ;;
            3)
                echo -e "\n${YELLOW}请输入新的 钉钉 Webhook (留空则不修改):${NC}"
                read -rp "Webhook: " new_webhook
                if [ -n "$new_webhook" ]; then
                    sed -i "s/DINGTALK_WEBHOOK=.*$/DINGTALK_WEBHOOK=\"$new_webhook\"/" "$CONFIG_FILE"
                    echo -e "${GREEN}钉钉 Webhook已更新${NC}"
                fi
                ;;
            4)
                echo -e "\n${YELLOW}请输入新的 主机备注 (留空则不修改):${NC}"
                read -rp "主机备注: " new_note
                if [ -n "$new_note" ]; then
                    sed -i "s/HOST_NOTE=.*$/HOST_NOTE=\"$new_note\"/" "$CONFIG_FILE"
